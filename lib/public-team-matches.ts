...
export function isRegularPhase(phase?: string | null) {
...
function sortRegisteredMatches<TMatch extends TeamMatchRecord>(a: TMatch, b: TMatch) {
  const jornadaDifference = (a.jornada ?? Number.MAX_SAFE_INTEGER) - (b.jornada ?? Number.MAX_SAFE_INTEGER);
  return jornadaDifference || a.id - b.id;
}

/**
 * Construye las dos vueltas esperadas contra cada rival. Los huecos se conservan
 * con match=null para poder mostrarlos aunque todavía no existan en la base de datos.
 */
  ...
  return teams
    ...
    .flatMap((opponent) => {
      ...
      for (const match of registered) {
        if ((match.vuelta === 'ida' || match.vuelta === 'vuelta') && !assigned[match.vuelta]) {
          assigned[match.vuelta] = match;
        } else {
          unassigned.push(match);
        }
      }

      for (const leg of ['ida', 'vuelta'] as const) {
        if (!assigned[leg]) assigned[leg] = unassigned.shift();
      }

      return (['ida', 'vuelta'] as const).map((leg) => ({
        key: `${teamId}-${opponent.id}-${leg}`,
        opponent,
        leg,
        match: assigned[leg] ?? null,
      }));
    });
}
